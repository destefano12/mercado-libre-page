...
-- Ojo con el signo del angulo sobre X: girando +θ la pantalla se
-- inclina hacia tu cara y la lente mira para abajo (que es donde esta
-- la hoja). Con -θ pasa al reves y el celu le saca fotos al techo.
...
PhoneModel.RAISED_C0 = CFrame.new(0.55, 0.35, -1.3)
	* CFrame.Angles(0, math.pi, 0)
...
PhoneModel.AIM_C0 = CFrame.new(0.3, -0.15, -1.55)
	* CFrame.Angles(0, math.pi, 0)
	* CFrame.Angles(math.rad(68), 0, 0)
...
function PhoneModel.attach(character: Model): (Model?, Weld?, BasePart?)
	...
	local existing = character:FindFirstChild("CelularEnMano")
	if existing then
		existing:Destroy()
	end

	-- No se puede llamar "Celular": asi se llama la Tool de la mochila,
	-- y el cliente terminaba enganchando la Tool en vez de la pantalla
	-- (por eso el celu se veia apagado).
	local model = Instance.new("Model")
	model.Name = "CelularEnMano"
