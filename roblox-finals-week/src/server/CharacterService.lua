...
--[[
	...
	`Rig.build` y `Rig.face` le ponen a todo el mundo un casquete `Pelo`,
	un `Flequillo` y un par de `Ceja`: es lo que hace que un alumno
	recien creado no sea un pelado sin cara. Cuando el jugador elige un
	peinado o unas cejas del carnet hay que sacar el rasgo base de en
	medio, o la silueta nueva sale montada sobre la vieja — dos gorros
	apilados, cuatro cejas.

	Se esconden en vez de destruirse porque esas piezas no llevan el
	atributo `Accesorio` — `stripAccessories` no las toca —, asi que si
	las borraramos no habria forma de recuperarlas al desequipar sin
	reaparecer al personaje. `dressStudent` las vuelve a mostrar al
	principio de cada pasada.
--]]
local function hideBase(character: Model, ...: string)
...
local function hideBaseHair(character: Model)
	hideBase(character, "Pelo", "Flequillo")
end

--[[
	Los anclajes de la cara, derivados igual que en `Rig.face`.

	Se recalculan aca en vez de exportarlos porque `Rig.face` los usa
	para el profesor tambien, con otras proporciones; lo que comparten
...
--[[
	...
	Van del color del pelo y no de un marron fijo: en el trailer el
	personaje de pelo violeta tiene las cejas violetas, y es lo que hace
	que la cabeza se lea como una sola pieza en vez de como una peluca
	apoyada encima.
--]]
local function brows(character: Model, skin: Rig.Skin, size: Vector3,
	angle: number, lift: number)
...
local ESTETICAS: { [string]: (Model, Rig.Skin) -> () } = {
	...
	-- ── cejas ──────────────────────────────────────────────────────
	--[[
		La pagina de ACCESSORIES del trailer es, en su mayoria, cejas:
		catorce casillas con formas distintas del mismo rasgo. Tiene
		sentido — con la cabeza a esta escala y los ojos tan grandes, la
		ceja es lo unico que le cambia la expresion a un personaje.

		Los seis estilos son la misma pieza con otras medidas y otro
		angulo, asi que van por `brows` y no repiten el `attach`.
	--]]
	cejas_rectas = function(character, skin)
		brows(character, skin, Vector3.new(A.cabeza.X * 0.24, A.cabeza.Y * 0.06, 0.07), 0, 0)
	end,
	cejas_finas = function(character, skin)
		brows(character, skin, Vector3.new(A.cabeza.X * 0.26, A.cabeza.Y * 0.03, 0.07), 4, 0)
	end,
	cejas_gruesas = function(character, skin)
		brows(character, skin, Vector3.new(A.cabeza.X * 0.3, A.cabeza.Y * 0.11, 0.08), 3, 0)
	end,
	cejas_arqueadas = function(character, skin)
		-- El arco se arma con dos piezas: el tramo largo inclinado
		-- hacia afuera y una cola corta que baja. Una sola barra
		-- rotada da un acento, no un arco.
		brows(character, skin, Vector3.new(A.cabeza.X * 0.2, A.cabeza.Y * 0.05, 0.07), -16, 0)
		for _, side in { -1, 1 } do
	...
	cejas_enojadas = function(character, skin)
		brows(character, skin, Vector3.new(A.cabeza.X * 0.26, A.cabeza.Y * 0.07, 0.07), 24, 0)
	end,
	cejas_unica = function(character, skin)
		hideBase(character, "Ceja")
		attach(character, "Head", "CejaEstetica",
	...
	-- ── marcas de la cara ──────────────────────────────────────────
	--[[
		Estas NO son exclusivas: se pueden llevar pecas y lunar y
		tirita a la vez, que es lo que uno espera de una cara. Por eso
		la categoria `marcas` no lleva `exclusiva` en `Config.Carnet`.
	--]]
	...
	pecas = function(character)
		-- Tres por mejilla, en diagonal. Simetricas exactas se ven
		-- estampadas; el desfase de la fila del medio alcanza.
		for _, side in { -1, 1 } do
			for i = 0, 2 do
				attach(character, "Head", "Peca",
					Vector3.new(A.cabeza.X * 0.035, A.cabeza.Y * 0.035, 0.06),
					CFrame.new(
	...
	bigote = function(character, skin)
		for _, side in { -1, 1 } do
...
function CharacterService.dressStudent(player: Player, character: Model, estetica: { [string]: boolean }?)
	local ok, err = pcall(function()
		...
		if head and head:IsA("BasePart") then
			...
			--[[
				Los rasgos de fabrica se restauran ANTES de vestir. Si el
				jugador se saco el peinado o las cejas del carnet, esto es
				lo que lo deja de nuevo con pelo y con cara en vez de
				pelado y sin cejas; si se los dejo puestos, la cosmetica
				los vuelve a esconder dos lineas mas abajo.
			--]]
			for _, child in character:GetChildren() do
				if child:IsA("BasePart") then
					if child.Name == "Pelo" or child.Name == "Flequillo" then
						child.Color = skin.pelo
						child.Transparency = 0
					elseif child.Name == "Ceja" then
						child.Transparency = 0
					end
				end
			end
